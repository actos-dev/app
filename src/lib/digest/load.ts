/**
 * Bülten sunucu yükleyicileri (Faz 5 / Birim 5A.3, U-08, S-15).
 *
 * `serverAuthApiFetchWithStatus` kullanılır: çerez forward edilir ve durum
 * kodu ayırt edilir; böylece "gerçekten bülten yok" (404 / boş dizi) ile
...
export async function loadCurrentDigest(): Promise<CurrentDigestResult> {
  ...
  const [current, windowResult] = await Promise.all([
    serverAuthApiFetchWithStatus<Digest>(digestServerPath()),
    serverAuthApiFetchWithStatus<Digest>(digestServerPath(), { query: { at } }),
  ]);
